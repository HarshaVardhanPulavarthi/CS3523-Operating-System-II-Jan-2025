//We are validating a sudoku by dynamic allocation of threads. We are using Bounded CAS method to prevent simultaneous accession of common variable
...
atomic<bool> cas_lock(false); //cas_lock 
...
// Defining the maximum CAS attempts before yielding
const int MAX_ATTEMPTS = 10; // Arbitrary bound -> we can adjust it based on needs
...
int main() {
    ...
    for (int i = 0; i < K; i++)
        threads.emplace_back(validateTask, i + 1);  // Thread IDs start from 1
...
// Critical Section - Now with bounded CAS Lock
chrono::high_resolution_clock::time_point casLock(int threadId) {
    ...
    int attempts = 0;
    while (true) {
        if (cas_lock.compare_exchange_strong(expected, true)) {
        ...
        if (attempts >= MAX_ATTEMPTS) {
            // Yield after reaching max attempts to avoid excessive spinning
            this_thread::yield();
...
void validateTask(int threadId) {
    ...
    while (C < 3*N && sudokuValid) { // Added early termination with sudokuValid
        // Critical Section
        auto enterTime = casLock(threadId);
        int startTask = C;
        C += taskInc;
...
//Function to check validity of single row
bool validateRow(int row) {
...
//Function to check validity of single column
bool validateCol(int col) {
    vector<int> section(N);
    for (int i = 0; i < N; i++)
...
//Function to check validity of single subgrid
bool validateGrid(int startRow, int startCol) {
    int n = sqrt(N);
    vector<int> section(N);
    int idx = 0;
